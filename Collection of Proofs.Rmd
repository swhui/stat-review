...
$$(W^{1/2} (Y-X\beta) )^{T} (W^{1/2} (Y-X\beta) ) = (Y^{T} - \beta^{T} X^{T}) (W^{1/2})^{T} W^{1/2} (Y-X\beta) = Y^{T} W Y - \beta^{T} X^{T} W Y - Y^{T} WX \beta + \beta^{T} X^{T} W X \beta$$
...
The main advantage of using WLS is the retainment of interpretations of the coefficients and interpretting F-tests and R-squared values (usually WLS keeps the intercept while transformed models do not). It is also advantageous to downweight outlier or influential points by setting its weight to 0.
